...
- 트래픽 분석: Google Analytics, Microsoft Clarity
- 수익화: Google AdSense
- 언어: 14개 다국어 지원 (ko, en, ar, fa, ja, es, tr, de, fr, pt-BR, it, zh-CN, zh-TW, ru), 기기 설정 언어 자동 감지 및 미지원 시 /en 폴백, 14개 언어 드롭다운 메뉴, 아랍어(ar)·페르시아어(fa) RTL 지원
...
- `NEXT_PUBLIC_GA_ID`에는 Google Analytics 웹 데이터 스트림의 측정 ID(`G-...`)를 넣는다. 숫자로 된 스트림 ID는 코드에서 사용하지 않는다.
...
- 프론트 robots: `frontend/src/app/robots.txt`
- 프론트 sitemap: `frontend/src/app/sitemap.ts`
- 전역 JSON-LD: `frontend/src/app/layout.tsx`
- `robots.txt`는 검색과 AI 입력 사용은 허용하고 AI 학습은 거부하는 `Content-Signal` 정책을 선언한다.
- `robots.txt`에 `/cdn-cgi/` Disallow를 설정하여 Bing 등 검색엔진 크롤러가 Cloudflare 프록시 내부 URL(`https://www.hrmz.today/cdn-cgi/...`)을 수집하면서 발생하는 메타 태그 누락 경고를 방지한다.
- Root Layout은 `WebSite`와 `Organization`을 JSON-LD로 제공하고 `publisher` 관계로 연결한다.
- Root Layout은 `next-intl`의 `getLocale()`을 바탕으로 `<html lang={locale}>` 동적 속성 및 `content-language` 메타 태그(`ko-KR`, `en-US`)를 자동 주입하여 Bing 등 검색엔진의 언어 태그 누락 지적을 예방한다.
- Root Layout의 Google Analytics 태그는 `NEXT_PUBLIC_GA_ID`가 있을 때만 로드되며, Microsoft Clarity(`clarityId`) 및 Google AdSense 스크립트가 추가되어 있다.
- sitemap 포함 페이지:
...
- Bing Webmaster Tools 지적사항에 따라 영문 페이지 Title은 70자 미만(메인 62자, About 67자, Events 55자, Sources 59자)으로 핵심 검색 키워드(Strait of Hormuz, US, Iran, Trump, Oil Price, Map 등)를 조합해 작성한다.
- SEO 키워드는 별도 `keywords` 메타와 title/description/본문 문맥에 자연스럽게 반영한다.
- Google Search Console, Naver Search Advisor, Bing Webmaster Tools 소유권 인증 값은 코드(`layout.tsx` verification 또는 `seo.ts`) 또는 DNS 방식으로 관리한다.
- about/source 페이지의 출처 목록은 실제 수집 코드와 어긋나지 않게 주기적으로 점검한다.
- 배포 후 페이지 소스에서 `application/ld+json`, `googletagmanager.com/gtag/js`, `clarity.ms`, `adsbygoogle.js`가 정상 출력되는지 확인한다.
- GA/Clarity 복원 확인은 각 서비스의 실시간 대시보드에서 접속 이벤트가 정상 인입되는지 확인한다.

CSP(Content Security Policy) 및 서드파티 스크립트 관리:

- `frontend/next.config.ts`의 `cspReportOnly` 배열에서 외부 스크립트 및 연결 허용 도메인을 관리한다.
- 허용 스크립트(`script-src`): `'self'`, `'unsafe-inline'`, `'unsafe-eval'`, TradingView, GTM, GA, Clarity (`https://www.clarity.ms`), Google AdSense (`https://pagead2.googlesyndication.com`, `https://tpc.googlesyndication.com`).
- 허용 연결(`connect-src`): Supabase, GA, Clarity (`https://www.clarity.ms`, `https://*.clarity.ms`), AdSense, DoubleClick.
- 새로운 외부 분석/서드파티 도구 추가 시 `next.config.ts` CSP 항목도 함께 업데이트해야 브라우저 보안 차단 없이 작동한다.

PageSpeed/서드파티 태그 운영:

- TradingView와 MarineTraffic은 대시보드 핵심 사용성에 직접 연결되므로 PageSpeed 점수만을 위해 클릭 후 로드 방식으로 바꾸지 않는다.
- PageSpeed의 서드파티 캐시 TTL 경고는 대부분 외부 서비스가 정한 헤더라 코드에서 직접 수정할 수 없다. 개선하려면 태그를 제거하거나 로드 조건을 바꿔야 한다.
- 광고/유료 마케팅을 운영하지 않는 동안 Facebook Pixel, HubSpot, Hotjar, LinkedIn Insight, ProfitWell, GrowthBook 태그는 유지하지 않는다.
- Cloudflare Web Analytics beacon과 GTM 컨테이너는 필요 여부를 운영자가 확인한 뒤 유지/제거한다.
- React hydration 오류를 막기 위해 서버와 브라우저에서 다르게 계산될 수 있는 시간 표시는 첫 렌더에서 UTC나 locale별 고정 시간대 같은 안정적인 기준으로 렌더한다.
- 접속자 기기 시간대 표시는 브라우저 hydration 이후에만 적용한다. 서버 렌더 단계에서 `Intl.DateTimeFormat().resolvedOptions().timeZone`을 직접 사용하지 않는다.
...
대시보드 HTML 자체는 별도 계층으로 캐시한다.

- `frontend/src/app/[locale]/page.tsx`: `export const revalidate = 60`
- `frontend/src/app/[locale]/layout.tsx`: `generateStaticParams()`로 `/en`, `/ko` 생성
- locale layout/page: `setRequestLocale(locale)` 호출
- production build 결과에서 `/en`, `/ko`가 `● SSG`, `Revalidate 1m`으로 표시되어야 한다.

이 두 계층은 역할이 다르다.

1. `unstable_cache`: Supabase 쿼리 결과를 데이터 종류별 TTL로 캐시한다.
2. 60초 ISR: 완성된 대시보드 HTML/RSC를 Vercel Full Route Cache에 보관한다.

데이터 캐시만 적용하고 페이지에 `force-dynamic`을 두면 첫 방문자는 캐시가 비어 있을 때 모든 Supabase 조회가 끝날 때까지 HTML을 받지 못한다. 따라서 대시보드에 `force-dynamic`을 다시 추가하지 않는다.
...
- `no-store` 쿼리는 반드시 `dashboard-cache.ts` wrapper 내부에서 호출한다. 서버 페이지에서 직접 호출하면 정적 생성/ISR이 동적 렌더링으로 후퇴할 수 있다.
- `next-intl` 라우트에서 `generateStaticParams`만 추가하는 것으로는 충분하지 않다. layout/page에서 locale을 params로 받고 `setRequestLocale(locale)`를 호출해야 요청 헤더 의존 동적 렌더링을 피할 수 있다.
- TTL을 너무 길게 하면 DB에는 새 데이터가 있어도 화면 반영이 늦어진다.
- 상황 요약은 첫 화면에 보이므로 너무 긴 TTL은 체감 품질을 해친다.

### 6.1 2026-06-12 첫 접속 지연 장애 기록

증상:

- 운영 첫 요청 TTFB 약 7.6초
- 직후 요청 TTFB 약 0.7초
- 응답 헤더가 `cache-control: private, no-cache, no-store`
- `x-vercel-cache: MISS`

원인:

- 대시보드에 `export const dynamic = "force-dynamic"`이 있어 매 요청 서버 렌더링됐다.
- 섹션별 `unstable_cache`는 있었지만 Full Route Cache가 비활성화돼 첫 캐시 미스 비용이 방문자 응답에 전가됐다.
- 최신 시장 스냅샷을 11개 심볼별로 각각 조회해 불필요한 Supabase 왕복이 있었다.
- 하이드레이션, Supabase RLS 보안 강화, Render 크론 통합은 직접 원인이 아니었다.

해결:

- `force-dynamic` 제거 후 60초 ISR 적용
- `/en`, `/ko` 정적 파라미터 생성
- layout/page에 `setRequestLocale(locale)` 적용
- 최신 시장 스냅샷 조회를 11회에서 단일 조회로 통합하고 날짜 내림차순 결과에서 심볼별 첫 행을 사용
- GitHub PR #56, main merge commit `49bb4baf373976ee0843eb31d791f35dbb56882f`

검증 결과:

- Next.js production build: `/en`, `/ko`가 `● SSG`, `Revalidate 1m`
- 운영 응답: `x-vercel-cache: PRERENDER`
- `/en` TTFB 약 0.48~0.57초
- `/ko` TTFB 약 0.50~0.60초
- `/` 리다이렉트 포함 약 0.68~0.90초

허용한 트레이드오프:

- 완성된 페이지는 최대 약 60초 늦을 수 있다.
- 갱신 시 기존 페이지를 제공하면서 백그라운드 재생성이 진행된다.
- 시장 수집은 5분, 선박은 약 20분, 뉴스/요약은 1시간 주기이므로 60초 지연은 운영상 허용한다.
...
9. 업데이트 시간은 첫 렌더에서 한국어 `Asia/Seoul`, 영어 `UTC`로 표시하고, hydration 이후 접속자 기기 시간대로 다시 표시한다.
...
1. `curl -D - -o /dev/null https://www.hrmz.today/en`으로 `x-vercel-cache`와 `cache-control`을 확인한다.
2. 정상 대시보드는 `x-vercel-cache: PRERENDER` 또는 Vercel 캐시 상태가 나오며, `private, no-cache, no-store`가 나오면 안 된다.
3. `npm run build` 결과에서 `/en`, `/ko`가 `● SSG`, `Revalidate 1m`인지 확인한다. `ƒ Dynamic`이면 ISR이 깨진 것이다.
4. `frontend/src/app/[locale]/page.tsx`에 `force-dynamic`이 다시 들어갔는지 확인한다.
5. locale layout/page의 `generateStaticParams`, `setRequestLocale`가 유지되는지 확인한다.
6. `frontend/src/lib/api/dashboard-cache.ts` wrapper를 우회해 Supabase query를 페이지에서 직접 호출하는지 확인한다.
7. Supabase 쿼리가 한 섹션에 몰려 있는지 `frontend/src/lib/api/dashboard.ts`를 확인한다.
8. 최신 시장 스냅샷은 심볼별 직렬 조회가 아니라 단일 조회인지 확인한다.
9. 하이드레이션 오류는 HTML 수신 이후 문제다. TTFB가 길다면 먼저 서버 렌더/캐시를 조사한다.
10. 캐시 TTL을 줄이면 최신성은 올라가지만 DB 호출이 늘고, 늘리면 반영이 늦어진다.
...
- 프론트 파일만 변경했으면 Vercel 배포 상태와 운영 캐시 헤더만 확인한다. Render 재배포는 필요 없다.
- `backend/**` 또는 `render.yaml`을 변경했을 때만 Render service/cron 최신 빌드를 확인한다.
- Supabase 최신 row 확인
- 실제 페이지에서 한/영 표시 확인
- 대시보드 캐시 변경 시 `/en`, `/ko`의 `x-vercel-cache`와 TTFB를 연속 측정한다.
