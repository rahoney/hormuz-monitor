...
export default function SituationSummaryCard({ summary }: Props) {
  ...
  let structuredToRender: StructuredSituationSummary | null = null;
  let text: string | null = null;

  if (summary) {
    if (locale === "ko") {
      structuredToRender = isStructuredSummary(summary.summary_ko_structured) ? summary.summary_ko_structured : null;
      text = summary.summary_ko;
    } else if (locale === "en") {
      structuredToRender = isStructuredSummary(summary.summary_en_structured) ? summary.summary_en_structured : null;
      text = summary.summary_en || summary.summary_ko;
    } else {
      // 12개 신규 언어: 사전 번역 데이터 사용
      if (summary.locale_translated === locale) {
        structuredToRender = isStructuredSummary(summary.summary_translated_structured) ? summary.summary_translated_structured : null;
        text = summary.summary_translated_text ?? null;
      }
      // DB에 없으면 영문 / 한글 fallback
      if (!structuredToRender && !text) {
        structuredToRender = isStructuredSummary(summary.summary_en_structured)
          ? summary.summary_en_structured
          : (isStructuredSummary(summary.summary_ko_structured) ? summary.summary_ko_structured : null);
        text = summary.summary_en || summary.summary_ko;
      }
    }
  }
